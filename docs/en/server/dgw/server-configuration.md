...
Installing by using the ***Side-by-side*** method creates a free {{ en.DGW }} that can be used for up to 5 concurrent sessions, or uses the {{ en.DGW }} licence(s) you have to support more sessions. This method is recommended for simpler network layouts, and only one ***Side-by-side*** installation can be done per machine.
...
1. If default values don't work for your environment, enter the desired settings.
...
Installing by using the ***Standalone*** method will allow to install {{ en.DGW }} on a separate server than Devolutions Server. It could be for performance purposes or access networks that Devolutions Server can't access. Many {{ en.DGW }} can be used by Devolutions Server, but only one {{ en.DGW }} can be installed on a machine.
1. From the {{ en.RDMSCONSOLE }}, click on the ***Companions*** tab.
1. In the {{ en.DGW }} section, click on ***Install***.
1. Choose ***Standalone Installation***.
1. Choose between ***Download version*** & ***Install from msi file***. [Pourquoi ne pas avoir cette information dans le side by side ?]
{% snippet icon.badgeHelp %}
It is possible to download an [.msi](https://devolutions.net/gateway/download) file to install the {{ en.DGW }} on an offline {{ en.DPS }}.
{% endsnippet %}  

1. Click ***Next***.
1. Enter the ***Access URI*** information. [Il faudrait probablement décrire ce qu'est l'access URI, dans le genre : The Access URI is the URI that you will used to reach your Devolutions Gateway, for example, https://gateway.contoso.com]
...
Do not share the private key with other users, as it can be used to decrypt the communication between a client and Devolutions Gateway. Only the public key should be shared.
{% endsnippet %}  

1. Click ***Next***.
1. Choose between ***Use public key from DVLS instance 'Devolutions Server'*** : this option is used if the installed {{ en.DGW }} is on the same server as the Devolutions Server;  
and ***From file***: If installing {{ en.DGW }} on another computer, download the public key from the Devolutions Server you want to pair with that {{ en.DGW }}.
1. Click ***Next***.
1. Click ***Install***. 
    [Je ne pense pas que l'on devrait documenter le response file ici. C'est quelque chose qui revient ici et là dans la console, cela pourrait être documenté de façon plus général à plus haut niveau et avoir simplement un lien à partir d'ici]
...
## Steps Needed for Both Installation Methods [Cette section n'est pas vrai pour un side by side. Tu n'as pas besoin d'aller ajouter le gateway dans la config]
