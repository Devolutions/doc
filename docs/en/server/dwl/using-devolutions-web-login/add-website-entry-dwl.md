...
This topic explains how to create an entry with your existing website credentials. If you have not yet created an account for the website, see [Create an account for a website with the {{ en.DWL }}](/server/dwl/using-devolutions-web-login/create-account-website-dwl/) instead. 
{% endsnippet %}
 
Website entries can be created with the {{ en.DWL }} in {{ en.DVLS }}. This type of entry is useful for saving your login credentials so that you do not have to remember them. These entries are also used by the {{ en.DWL }} to recognize a website and [retrieve your credentials](/server/dwl/using-devolutions-web-login/retrieve-credentials-dwl/).  

The main way to achieve this is by successfully logging into the website. The {{ en.DWL }} will automatically offer to save your credentials in a new website entry in {{ en.DVLS }}. It is also possible to manually create the website entry.  
...
![Login Page](https://webdevolutions.azureedge.net/docs/en/server/ServerOp2009.png) 
1. Click on the {{ en.DWL }} icon in your browser toolbar and, in the ***Matching*** tab, click on the ***Add Website*** button. 
