---
eleventyComputed:
  title: OTP autofill with the {{ en.DWL }}
  description: The {{ en.DWL }} allows to autofill a one-time password (OTP) request with a Website session entry and a Website (Legacy) information entry from {{ en.RDM }}.
---
The {{ en.DWL }} allows to autofill a one time password (OTP) request with a ***Website*** session entry and a ***Website (Legacy)*** information entry from {{ en.RDM }}.
