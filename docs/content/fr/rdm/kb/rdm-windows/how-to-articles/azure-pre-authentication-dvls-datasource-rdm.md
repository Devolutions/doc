...
eleventyComputed:
  ...
  description: L'authentification préalable Azure nécessite une configuration très différente de celle par défaut. Pour utiliser cette fonctionnalité avec {{ fr.RDM }}, l'authentification doit être activée sur le {{ fr.DVLS }} souhaité, plusieurs configurations doivent être effectuées dans le centre d'administration Microsoft Entra, et un proxy d'application Azure doit être configuré.
---
...
- [Configurer un proxy d'application Azure](#configurer-un-proxy-d'application-azure)
- [Configurer {{ fr.RDM }}](#configurer-remote-desktop-manager)
- [S'assurer que TLS 1.2 est activé](#s'assurer-que-tls-1.2-est-activé)
...
1. Une fois le service connecteur téléchargé, exécuter le programme d'installation. Se connecter avec l'invite Azure, et attendre que l'installation se termine (le service peut être affiché comme indisponible pendant ce temps).
...
- L'URL de {{ fr.DVLS }} n'est pas la racine, c'est-à-dire https://myserver.com/dvls et non https://myserver.com
{% snippet icon.badgeInfo %}
Tout changement peut prendre 3-5 minutes pour prendre effet. Soyez conscient de cela lors des tests de modifications.
{% endsnippet %}
...
1. Ensuite, se rendre à ***Microsoft Entra ID*** – ***Enterprise applications***, cliquer sur l'inscription d'application nouvellement créée, aller à ***Application Proxy***, et remplir les champs requis :
...
4. Ensuite, aller à l'onglet ***Avancé***. S'assurer que toutes les options sont décochées ou définies par défaut, sauf ***Valider le certificat SSL du backend*** qui peut être laissé activé si nécessaire.
...
6. Dans l'écran ***Authentification***, cliquer sur le bouton ***Ajouter une plateforme***. Trois nouvelles plateformes doivent être créées : ***Web***, ***Application à page unique***, et ***Applications mobiles et de bureau***. Commençons par créer une plateforme ***Web***.
...
13. Cliquer sur le bouton ***Accorder le consentement d'administration pour le locataire*** et confirmer en cliquant sur ***Oui***.
![Accorder le consentement d'administration pour le locataire](https://cdnweb.devolutions.net/docs/INTERFACE4020.png)
14. Ensuite, naviguer vers ***Exposer une API*** et cliquer sur le lien ***Ajouter*** à côté de ***URI d'ID d'application***. Puis entrer la même URI que l'***URL externe*** de votre ***Proxy d'application***.
...
15. Cliquer sur ***Ajouter une portée*** et entrer les informations suivantes :
- **Nom de la portée** : user_impersonation
...
![Ajouter une portée – étape 1](https://cdnweb.devolutions.net/docs/INTERFACE4024.png)
![Ajouter une portée – étape 2](https://cdnweb.devolutions.net/docs/INTERFACE4025.png)
16. Aller à ***Rôles d'application*** et cliquer sur le bouton ***Créer un rôle d'application***. Deux rôles doivent être créés : **Utilisateur** et **msiam_access**.
![Créer un rôle d'application](https://cdnweb.devolutions.net/docs/INTERFACE4026.png)
17. Sur l'écran ***Créer un rôle d'application***, entrer les informations suivantes :
...
Et cocher le cas sous ***Voulez-vous activer ce rôle d'application ?***.
18. Avec le premier rôle créé, cliquer à nouveau sur ***Créer un rôle d'application*** et entrer les informations suivantes :
- **Nom d'affichage** : msiam_access
- **Types de membres autorisés** : Utilisateurs/Groupes
- **Valeur** : msiam_access
- **Description** : msiam_access
![Remplir les informations msiam](https://cdnweb.devolutions.net/docs/INTERFACE4032.png)
Et cocher le cas sous ***Voulez-vous activer ce rôle d'application ?*** comme précédemment.
...
Puis cocher ***Utiliser le proxy de pré-authentification*** et, optionnellement, ***Utiliser le proxy de pré-authentification pour la connexion DVLS*** (si ***Authentification Microsoft*** est déjà coché).
3. La configuration est alors terminée, et une connexion Microsoft Azure devrait apparaître dans une fenêtre intégrée ou un navigateur système (peut se produire deux fois si une fenêtre intégrée apparaît et une fois de plus dans un navigateur système avant l'authentification DVLS). Une fois cela fait, une invite pour l'authentification DVLS devrait apparaître.
...
## Ne pas supprimer CWAP_AuthSecret
Il y a un secret qui est configuré automatiquement lors de la création de l'***Application Proxy App***. Supprimer cela brisera la pré-authentification et peut nécessiter la recréation de l'ensemble du proxy d'application.
