...
eleventyComputed:
  ...
  description: L'authentification préalable Azure nécessite une configuration très différente de celle par défaut. Pour utiliser cette fonctionnalité avec {{ fr.RDM }}, l'authentification doit être activée sur le {{ fr.DVLS }} désiré, plusieurs configurations doivent être effectuées dans le centre d'administration Microsoft Entra, et un proxy d'application Azure doit être configuré.
---
...
Il y a un secret qui est configuré automatiquement lors de la création de ***Application Proxy App***. Supprimer cela brisera la pré-authentification et peut nécessiter la recréation de l'ensemble du proxy d'application.